    ...
    <div class="col-12" style="margin-top: -15px">
      ...
      <div class="row mt-2">
        @for ($i = 1; $i <= $tiket->jumlah; $i++)
          @php
            $array = array('kursi' => 'K' . $i, 'tempat' => $data['id'], 'waktu' => $data['waktu']);
            $cekData = json_encode($array);
          @endphp
          @if ($tiket->kursi($cekData) != null)
            <div class="col-lg-2 col-md-3 col-sm-4 col-6 mb-4">
              <a href="{{ route('pesan', ['kursi' => 'K' . $i, 'data' => Crypt::encrypt($data)]) }}">
                <div class="kursi bg-white">
                  <div class="font-weight-bold text-success m-auto" style="font-size: 26px;">K{{ $i }}</div>
                </div>
              </a>
            </div>
          @else
